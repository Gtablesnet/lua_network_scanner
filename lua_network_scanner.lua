...
function get_gps_coordinates(timeout)
    ...
    if not gps_data or gps_data == "" then
        coroutine.resume(co, {error = "No GPS data received."})
        return
    end
    ...
    if lat and lon then
        ...
        coroutine.resume(co, {error = "Failed to parse GPS data."})
    end
end
...
local function main()
    ...
    local coGps = coroutine.create(function()
        get_gps_coordinates(1)
    end)

    local coWifi = coroutine.create(function()
        local commandOutput = executeCommand("nmcli dev wifi list ifname wlan0")
        return parseWifiList(commandOutput)
    end)

    -- Execute both coroutines concurrently
    local resultGps, resultWifi
    coroutine.resume(coGps)
    coroutine.resume(coWifi)

    -- Wait for results
    while coroutine.status(coGps) ~= "dead" or coroutine.status(coWifi) ~= "dead" do
        -- Allow the coroutines to yield
        coroutine.yield()
    end

    resultGps = coroutine.resume(coGps)
    resultWifi = coroutine.resume(coWifi)

    -- Process GPS and Wi-Fi data separately
    local currentResult = nil
    if resultWifi then
        currentResult = resultWifi
    end

    -- Check if wifi networks were found
    if currentResult then
        print("WiFi Networks with signal:")
        for _, row in ipairs(currentResult) do
            local ssid = row[1]
            ...
            if not alreadyExists then
                local gps_coords = resultGps[1] or {}
...
while true do
    main()
    os.execute("sleep 1")  -- Sleep for 1 second before running again
end
